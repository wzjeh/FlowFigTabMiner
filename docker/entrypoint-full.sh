#!/bin/bash
# Fetch the weights, then hand over to the pipeline CLI or the demo page.
set -e

MODELS=/app/models
WEIGHTS_REPO="${FFTM_WEIGHTS_REPO:-wyzhaoc/FlowFigTabMiner-models}"
READY="$MODELS/.ready"          # the demo's job worker waits for this file
...
if [ "$1" = "web" ]; then
    shift
    # The page and its precomputed examples need no weights: serve them at once
    # (a scaled-to-zero Cloud Run instance must answer a QR-code scan quickly)
    # and fetch the weights alongside; extraction jobs wait for $READY.
    rm -f "$READY"
    export FFTM_MODELS_READY="$READY"
    ( until fetch_weights; do echo "[entrypoint] weight download failed, retrying in 30 s"; sleep 30; done
      touch "$READY"; echo "[entrypoint] weights ready" ) &
