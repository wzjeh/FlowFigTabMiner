...
有机锂在溶液中**从不以单体存在**。聚集态取决于溶剂、位阻和温度：

| 有机锂 | 烃类溶剂 | Et₂O | THF | 参考 |
|---|---|---|---|---|
| MeLi | 四聚体 | 四聚体 | — | [15] |
| n-BuLi | **六聚体** | 四聚体 | **二聚体** | [15,17] |
| s-BuLi | — | 二聚体 | 二聚体 | [17] |
| t-BuLi | 四聚体 | 二聚体 | **单体** | [17] |
| PhLi | — | 二聚体 | **二聚体** | [15] |
| LDA (iPr₂NLi) | — | — | **二溶剂化二聚体** (A₂S₂) | [14] |

**规律**：
- 溶剂配位能力越强（烃 < Et₂O < THF < HMPA）→ 聚集度越低 [14]
- 位阻越大（Me < n-Bu < s-Bu < t-Bu）→ 聚集度越低 [15]
- 温度越高 → 倾向解聚（熵有利）
...
(RLi)ₙ  ⇌  n/m (RLi)ₘ     (m < n)
          K_deagg
例：(PhLi)₂  ⇌  2 PhLi (单体)     THF 中
    (n-BuLi)₆  ⇌  3 (n-BuLi)₂     Et₂O 中
```

### 8.3 聚集态对动力学的影响

Collum [14] 的核心公式——速率方程揭示 TS 聚集态：
...
我们的 75 个有机锂中间体在 THF/hexane 4:1 中的聚集态**未知**。可能的情况：

| 类别 | 可能聚集态（THF 中） | 对 Ea_obs 的影响 |
|---|---|---|
| ArLi (PhLi 类) | 二聚体 | Ea_obs = Ea_intrinsic + ΔH_deagg（若经单体路径）|
| oxiranylLi | 不确定（小分子，可能二聚/四聚） | 取决于取代基位阻 |
| alkylLi (n-BuLi 类) | 二聚体 | 同上 |
| carbenoid (LiCHXY) | 可能二聚体 [7] | Ramachandran 专门研究了二聚体 TS |

**关键**：我们的模型隐式假设"所有化合物在同一溶剂中聚集行为相似"。这在**类别内**可能成立（结构相似 → 聚集态相似），但**跨类别**不成立——这可能是 LOCO 验证失败的部分原因。

### 8.5 hexane 共溶剂的作用

我们的实验溶剂是 THF/hexane 4:1（体积比），不是纯 THF：
- hexane 不配位 Li → 降低有效溶剂浓度
- THF 的有效浓度 ≈ 80% × 12.3 M = 9.8 M（远高于 Li⁺ 浓度）
- 因此 Li 的第一配位层仍以 THF 为主（选择性溶剂化）
- 但 hexane 的存在降低了介电常数（ε_THF = 7.5 → ε_mix ≈ 5-6）
- 这可能影响离子对分离和聚集平衡
...
| 判断 | 证据 | 文献支撑 |
|---|---|---|
| **类别特异建模** | LOCO R² < -12，跨类失败 | Collum [14]: "substrate-dependent mechanisms may be the rule" |
| **xTB 优于 DFT 描述符** | xTB R²=0.69 > M06-2X 0.41 | 瓶颈在溶液相复杂性 [14]，不在气相精度 |
| **CPCM 不足以修正气相 TS** | 仅回收 23% 焓差距 | Ramachandran [7]: 显式 3THF 效应 = -31 kJ/mol |
| **ΔS‡ 符号区分机制** | 缔合(-121) vs 解离(+77) | 与 Collum [14] Table 1 一致 |
| **QSPR 是务实路线** | R² = 0.61–0.94 (类别内) | TS 计算需天-周/化合物，不可扩展 |
...
- 实验 Ea 是**表观值**，= Ea_intrinsic + ΔH_deagg [14]
- Ramachandran [7] 定量：聚集态升高势垒 **+24 kJ/mol**，溶剂化降低 **-31 kJ/mol**
- 我们的描述符全部计算在**气相单体**上，未包含聚集态信息
- **部分缓解**：类别内聚集态可能相似（结构类似 → 聚集行为类似），所以类别内模型仍有效
- **潜在改进**：加入聚集态相关描述符（如分子量、极化率、Li 暴露面积）

#### 问题 2：ΔS‡ 归因过于简单

```
我们说:   ΔS‡ = -121 → "溶剂重组"
实际上:   ΔS‡ = ΔS_deagg(+) + ΔS_solvent(-) + ΔS_TS_conform(-) + ΔS_mixed_agg(±)
```

- ΔS‡ 是多个对抗效应的净值 [14]
- 不同化合物的各组分比例不同 → 不能简单地用一个描述符（如 mol_volume）代理
- **后果**：这是 lnA 无法从基态描述符预测的根本原因
- **影响范围**：主要影响 t½ 模型（Ea 模型不受 ΔS‡ 影响）

#### 问题 3：溶剂化影响被单向理解

```
我们说:   "溶剂稳定 TS → 降低势垒"
Collum [14]: "只考虑 TS 端 is complete nonsense"
实际上:   ΔΔG‡_solv = ΔG_solv(TS) - ΔG_solv(GS)
          可正可负！
```

- 溶剂同时稳定基态和过渡态，净效应取决于差值 [14, Section 3.8-3.11]
- 有时弱配位溶剂反而加速反应（Collum Section 3.9: "weakly coordinating solvents can accelerate reactions"）
- **后果**：我们的 Gsolv 描述符只编码了基态溶剂化能，不包含 TS 溶剂化信息
- **但**：Gsolv 在 oxiranylLi t½ 模型中表现优异 (R²=0.94)，说明对该类别基态溶剂化确实是决定因素
...
高可信度 ✓                         低可信度 ✗
├─ 类别内 Ea 排序                   ├─ 跨类别预测
├─ oxiranylLi t½ (R²=0.94)         ├─ 绝对 Ea 值（含系统偏差）
├─ o-ArLi Ea (σ+B5, R²=0.91)      ├─ lnA / ΔS‡ 的物理解释
└─ 机制异质性的判断                 └─ 新结构类别的外推
```
...
3. **溶液相动力学复杂性** [14]：速率方程揭示 TS 聚集态、溶剂影响两端（GS+TS）、底物依赖机制
4. **数据集**：75 个中间体，40 Arrhenius 参数，5 个结构类别
5. **描述符筛选**：xTB → HF → M06-2X → QTAIM 四个层级，xTB 最优（R²=0.69 > DFT 0.41）
6. **LOCO 验证**：跨类别预测 R² < -12 → 机制异质性（文献支撑：Collum [14] "substrate-dependent mechanisms"）
7. **Ea-lnA 补偿与聚集态**：解聚同时贡献 +Ea 和 +ΔS‡（Ramachandran [7] 定量：+24/-31 kJ/mol）
8. **类别特异模型**：o-ArLi R²=0.91, oxiranylLi R²=0.92/0.94，各类别描述符的物理意义
9. **TS 计算验证**：三个体系 × 两个溶剂化层级，气相高估 2.6-4.1×，CPCM 仅修正 23%
10. **建模方法论与自我审视**：流程图 + 三个待解决问题（聚集态/ΔS‡/溶剂化）+ 已验证的判断
11. **可信度边界**：模型在什么范围内可信（类别内排序 ✓）、什么范围外不可信（跨类/外推 ✗）
12. **结论与展望**：QSPR 是当前最优路线；更多实验数据 >> 更高级计算；聚集态描述符是潜在改进方向
...
[17] Lucht, B. L.; Collum, D. B. "Lithium Ion Solvation: Amine and Unsaturated Hydrocarbon Solvate Structures and Energies." *Acc. Chem. Res.* **1999**, 32, 1035–1042. — 有机锂溶液结构综述：n-BuLi 在不同溶剂中的聚集态（六聚体→四聚体→二聚体），NMR 表征方法。

[18] Reich, H. J. "Role of Organolithium Aggregates and Mixed Aggregates in Organolithium Mechanisms." *Chem. Rev.* **2013**, 113, 7130–7178. — 有机锂聚集态与反应性的全面综述，NMR 快速注入法测定溶液聚集态。

[19] Harrison-Marchand, A.; Mongin, F. "Mixed AggregAte (MAA): A Single Concept for All Dipolar Organometallic Aggregates." *Chem. Rev.* **2013**, 113, 7470–7562. — 混合聚集体综述，解释锂盐/副产物如何改变反应路径。
