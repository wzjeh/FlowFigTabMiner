...
{{
  ...
  "reaction_class": "...",         // REQUIRED — choose ONE from: "nucleophilic addition" | "halogen-metal exchange" | "directed metalation" | "anionic cyclization" | "C-C coupling" | "C-N coupling" | "C-O coupling" | "polymerization" | "halogenation" | "oxidation" | "reduction" | "hydrogenation" | "esterification" | "amidation" | "alkylation" | "acylation" | "photocatalysis" | "hydrolysis" | "other". Must not be null. Use "other" if unsure. IMPORTANT: this is a PAPER-LEVEL main transformation label and must be the same for all records from the same paper. Choose the dominant organolithium-enabled transformation from the paper title/text, not a downstream row-specific trapping step. Examples: halogen-lithium exchange followed by trapping -> "halogen-metal exchange"; deprotolithiation / lithiation-substitution papers -> "directed metalation"; carbamoyllithium papers focused on amidation -> "amidation"; polymer papers -> "polymerization".
...
13. REACTION CLASS: reaction_class MUST be filled for every record. Read the paper title/text, identify the single main organolithium-enabled transformation of the paper, and assign that SAME class to every record from the paper. Do not switch classes across rows just because different electrophiles, trapping products, or downstream steps appear. Use "other" if none fit. Never leave reaction_class as null.
